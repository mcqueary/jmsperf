...
if [ $FLAVOR = "tibems" ]; then
  export CLASSPATH=jmsperf.jar:${TIBEMS_CLASSPATH}:provider/tibems/jndi.properties
else
  export CLASSPATH=jmsperf.jar:${WMQ_CLASSPATH}:provider/wmq/jndi.properties
fi

echo java -cp $CLASSPATH $TARGETCLASS -propFile $PROPERTIES
